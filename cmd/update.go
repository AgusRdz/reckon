...
const repo = "AgusRdz/reckon"

// Update checks for a newer version and replaces the binary if one is found.
func Update(current string) {
	latest, err := latestVersion()
	if err != nil {
		fmt.Fprintf(os.Stderr, "reckon: failed to check for updates: %v\n", err)
		os.Exit(1)
	}

	if latest == current || (current != "dev" && normalize(latest) == normalize(current)) {
		fmt.Printf("reckon %s is already up to date\n", current)
		return
	}

	if current == "dev" {
		fmt.Printf("reckon dev build — latest release is %s\n", latest)
		fmt.Println("install a release build to enable updates")
		return
	}

	fmt.Printf("updating reckon %s → %s...\n", current, latest)

	url := assetURL(latest)
	exe, err := os.Executable()
	if err != nil {
		fmt.Fprintf(os.Stderr, "reckon: cannot determine executable path: %v\n", err)
		os.Exit(1)
	}
	exe, _ = filepath.EvalSymlinks(exe)

	if err := downloadReplace(url, exe); err != nil {
		fmt.Fprintf(os.Stderr, "reckon: update failed: %v\n", err)
		os.Exit(1)
	}
...
func assetURL(version string) string {
	...
	binary := fmt.Sprintf("reckon-%s-%s%s", goos, arch, ext)
...
func downloadReplace(url, dest string) error {
	...
	if runtime.GOOS == "windows" {
		...
		if err := os.Rename(tmp, dest); err != nil {
			os.Rename(old, dest) // best-effort rollback
			return fmt.Errorf("cannot replace binary: %w", err)
		}
		// .old will be cleaned up on next update or can be ignored
		return nil
	}
